...
`;
  ...
  private getAgentsContent(n8nVersion: string): string {
    return [
      ...
      ``,
      `1. **LIST FIRST**: Check workflow status with \`n8nac list\` or \`n8nac list --local\`/\`--remote\``,
      `   - Understand which workflows exist locally vs remotely`,
      `   - Identify workflow IDs and their sync status`,
      ``,
      `2. **FETCH REMOTE STATE**: Update your local cache of remote state`,
      `   - \`n8nac fetch <workflow-id>\`: Fetch specific workflow's remote state`,
      `   - \`n8nac fetch --all\`: Fetch all remote workflows' state`,
      `   - This updates internal comparison cache without downloading files`,
      ``,
      `3. **PULL IF NEEDED**: Download remote changes before editing`,
      `   - \`n8nac pull --id <workflow-id>\`: Download workflow from n8n to local`,
      ...
      `   - \`n8nac push --id <workflow-id>\`: Upload local workflow to n8n`,
